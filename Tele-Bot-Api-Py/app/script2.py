...
async def process_bot(bot):
    ...
    try:
        ...
        selected_groups = random.sample(groups, min(11, len(groups)))
...
async def main():
    while True:
        ...
        if online_bots:
            ...
            bot_chunks = [online_bots[i:i + 5]
                          for i in range(0, len(online_bots), 5)]
            for chunk in bot_chunks:
                tasks = [process_bot(bot) for bot in chunk]
                await asyncio.gather(*tasks)
                print(f"Processed a chunk of {len(chunk)} bots.")
